      ...
      <div className="border-border flex flex-col gap-3 border-b pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-text-secondary mt-1 text-xs">
            Generate regulatory audit trails, export SAR-eligible cases, and
            track investigation resolution metrics.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="border-border bg-surface relative overflow-hidden rounded-xl border p-5 shadow-xs transition-all hover:shadow-md">
          ...
          <div className="mt-3 flex items-baseline justify-between">
            ...
            <span className="rounded-full border border-rose-500/20 bg-rose-500/10 px-2 py-0.5 text-xs font-semibold text-rose-700 dark:text-rose-300">
              Confirmed Suspicious
            </span>
          </div>
          <p className="text-text-secondary mt-2 text-[11px]">
            Ready for regulatory dossier export
          </p>
        </div>
      ...
      <div className="border-border bg-surface flex flex-col gap-4 rounded-xl border p-6 shadow-xs">
        <div className="border-border flex items-center justify-between border-b pb-3">
          ...
          <span className="text-text-secondary font-mono text-[11px]">
            ENCRYPTED CSV REGISTERS
          </span>
        </div>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="border-border flex flex-col justify-between gap-4 rounded-xl border bg-slate-50/70 p-5 dark:bg-slate-900/50">
            <div>
              ...
              <p className="text-text-secondary mt-1 text-xs leading-relaxed">
                Comprehensive CSV register containing all triggered alerts, risk
                tier categories, disposition notes, and investigator signoff
                timestamps.
              </p>
            </div>
            <button
              ...
              className="bg-brand-navy hover:bg-brand-blue inline-flex items-center gap-1.5 self-start rounded-lg px-4 py-2 text-xs font-semibold text-white shadow-sm transition-all duration-150 active:scale-95 disabled:opacity-50"
            >
          ...
          <div className="border-border flex flex-col justify-between gap-4 rounded-xl border bg-slate-50/70 p-5 dark:bg-slate-900/50">
            <div>
              ...
              <p className="text-text-secondary mt-1 text-xs leading-relaxed">
                Full ledger export of analyzed transactions, clearing channels,
                transaction volumes, ML scoring flags, and graph indicators.
              </p>
            </div>
            <button
