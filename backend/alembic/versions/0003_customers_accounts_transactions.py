...
def upgrade() -> None:
    ...
    op.create_index("idx_customers_risk_category", "customers",
                    ["current_risk_category"], schema="public")
    op.create_index("idx_customers_external_ref", "customers",
                    ["external_ref"], schema="public")
    ...
    op.create_table(
        ...
        sa.Column("uploaded_by", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("public.users.user_id"), nullable=False),
    ...
    op.create_index("idx_ingestion_batches_user", "ingestion_batches",
                    ["uploaded_by"], schema="public")
    op.create_index("idx_ingestion_batches_status", "ingestion_batches",
    ...
    op.create_table(
        ...
        sa.Column("origin_account_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("public.accounts.account_id"), nullable=False),
        sa.Column("destination_account_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey("public.accounts.account_id"), nullable=False),
        ...
        sa.Column("ingested_at", sa.DateTime(timezone=True), nullable=False,
                  server_default=sa.func.now()),
        sa.Column("ingestion_batch_id", postgresql.UUID(as_uuid=True),
    ...
    op.create_index("idx_txn_origin", "transactions",
                    ["origin_account_id"], schema="public")
    op.create_index("idx_txn_dest", "transactions",
                    ["destination_account_id"], schema="public")
    op.create_index("idx_txn_occurred_at", "transactions",
                    ["occurred_at"], schema="public")
    op.create_index("idx_txn_batch", "transactions",
                    ["ingestion_batch_id"], schema="public")
...
def downgrade() -> None:
    # Drop RLS policies
    op.execute('drop policy if exists "Data operators and admins can create ingestion batches" on public.ingestion_batches')
    op.execute('drop policy if exists "Authenticated users can view ingestion batches" on public.ingestion_batches')
    op.execute('drop policy if exists "Authenticated users can view transactions" on public.transactions')
